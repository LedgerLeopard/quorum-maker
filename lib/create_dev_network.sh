...
function copyStartTemplate(){
    ...
    cp lib/dev/tessera-config_template.json $projectName/node$1/node/tessera-config.json
    sed -i "s/#mNode#/node$1/g" $projectName/node$1/node/tessera-config.json
    sed -i "s/#localIp#/${DOCKER_NETWORK_IP}$(($1+1))/g" $projectName/node$1/node/tessera-config.json
    COMMA=","
    if [ $i -eq $nodeCount ]; then
        COMMA=""
    fi
    peers="$peers{\"url\":\"http://${DOCKER_NETWORK_IP}$(($1+1)):22002/\"}$COMMA"
}
...
function createAccount(){
    ...
    balances="$balances\"$mAccountAddress\": {\"balance\": \"1000000000000000000000000000\"}$COMMA"
...
function generateGenesis(){
    ...
    PATTERN="s/#AccountBalance#/$balances/g"
    sed -i $PATTERN $projectName/genesis.json
}
...
function addPeers(){
    i=1
    while : ; do
        sed -i "s/#peer#/$peers/g" $projectName/node$i/node/tessera-config.json
...
function main(){
    ...
    echo ""
    (printf "NODE PUBLIC-KEY IP RPC WHISPER  TESSERA RAFT NODEMANAGER WS\n" \
